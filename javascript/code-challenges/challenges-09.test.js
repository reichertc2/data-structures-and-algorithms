...
const checkValues = (obj, value) => {
  // Solution code here...
  let found = false;
  let valueArr = Object.values(obj).forEach(el => el === value ? found = true : "");
  return found;
};
...
const updateNumbers = (obj) => {
  // Solution code here...
  let localArray = [];
  Object.keys(obj).forEach(key => localArray.push(`${key}: ${obj[key]}`));
  return localArray;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(child => {
    if (child.name === character) {
      Object.keys(child).forEach((key, idx) => {
        if (key === 'children') kids = Object.values(child)[idx].length;
      });
    }
  });
